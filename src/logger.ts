	...
	if (level <= logLevel) {
		if (level === LogLevels.ERROR)
			return console.error(
				`${textRed(icons[0])}${reset('')} ${bgRed(textBold(' ERROR '))}${reset('')} ${textBold(
					message
				)}`
			);
		if (level === LogLevels.WARNING)
			return console.warn(
				`${textYellow(icons[1])}${reset('')} ${bgYellow(textBold(textBlack(' WARNING ')))}${reset(
					''
				)} ${textBold(message)}`
			);
		console.log(
			`${textWhite(icons[2])}${reset('')} ${bgWhite(textBold(textBlack(' VERBOSE ')))}${reset(
				''
			)} ${textBold(message)}`
		);
	}
...
export function logTitle(title: string, message: any, level = LogLevels.VERBOSE) {
	const icons = ['✘', '▲', '!'];
...
export function logError(error: any) {
	...
	return console.error(
		`${textRed('✘')}${reset('')} ${bgRed(textBold(' ERROR '))}${reset('')} ${textBold(
			`${name}: ${message}`
		)}\n${textBold('Stacktrace:')}${reset('')} ${stack}`
	);
}
