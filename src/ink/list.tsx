    ...
    <Box flexDirection="column" paddingLeft={0.5} paddingRight={0.5} paddingTop={1}>
      ...
      <Box justifyContent={'space-between'} paddingTop={1}>
        <Box width={fixedWidth}>
          <Text bold color={'cyanBright'}>app name</Text>
        </Box>
        ...
        <Box width={fixedWidth}>
          <Text bold color={'cyanBright'}>cpu</Text>
        </Box>
  
        <Box width={fixedWidth}>
          <Text bold color={'cyanBright'}>memory</Text>
        </Box>
  
        <Box width={fixedWidth}>
          <Text bold color={'cyanBright'}>uptime</Text>
        </Box>
  
        <Box width={fixedWidth}>
          <Text bold color={'cyanBright'}>status</Text>
        </Box>
      </Box>

      {infos.length === 0 ? <Box width={'100%'} paddingTop={1}>
        <Text color={'cyanBright'}><Spinner /></Text>
      </Box> : null}
  
      {
        infos.map((app: AppInfo) => (
          <Box key={app.appName} justifyContent={'space-between'} paddingTop={1}>
            <Box width={fixedWidth}><Text>{app.appName}</Text></Box>
            <Box width={fixedWidth}><Text>{app.port}</Text></Box>
            <Box width={fixedWidth}><Text>{cpu(app.cpu ?? 0)}</Text></Box>
            <Box width={fixedWidth}><Text>{memory(app.memory ?? 0)}</Text></Box>
            <Box width={fixedWidth}><Text>{time(app.uptime)}</Text></Box>
            <Box width={fixedWidth}><Text color={statusToColor(app.status ?? 'stopped')}>{app.status ?? 'stopped'}</Text></Box>
          </Box>
        ))
      }
    </Box>
