...
export const getLogFilePath = (deployment: Deployment) =>
  join(
    ...
    'logs',
    deployment.name,
    deployment.logFilePath ?? `stdout.log`,
  );
export const getErrorLogFilePath = (deployment: Deployment) =>
  join(
    ...
    'logs',
    deployment.name,
    deployment.errorLogFilePath ?? `stderr.log`,
  );
...
export async function deploy(projectName: string) {
  ...
  for (let i = 0; i < config.deployments.length; i++) {
    // change dir
    process.chdir(
      join(ROOT_CIRRUS_PATH, projectName, config.deployments[i].path),
    );
    // run install
...
export const getWorkingDir = (projectName: string) =>
...
export function writeHook(projectName: string) {
  ...
  writeFileSync(
    join(ROOT_CIRRUS_PATH, projectName, '.git', 'hooks', 'post-receive'),
    hook,
  );
  chmodSync(
    join(ROOT_CIRRUS_PATH, projectName, '.git', 'hooks', 'post-receive'),
