...
test('create > monorepo options', async (t) => {
	...
	t.throws(() => createRepoDefinition('monorepo', t.context.monorepo, monorepoGlobalOptions));
				...
				apps: [
					{
						...
						errorFile: join(__testDirname, 'cirrus', 'logs', 'monorepo', 'monorepo-app.error.log'),
						logFile: join(__testDirname, 'cirrus', 'logs', 'monorepo', 'monorepo-app.log'),
						env: { NODE_ENV: 'test' },
						path: 'apps/web',
					...
					},
					{
						...
						errorFile: join(__testDirname, 'cirrus', 'logs', 'monorepo', 'monorepo-api.error.log'),
						logFile: join(__testDirname, 'cirrus', 'logs', 'monorepo', 'monorepo-api.log'),
						env: {
							NODE_ENV: 'production',
							SECRET_KEY: 'abcd'
						},
...
test('create > app options', async (t) => {
	const globalOptions = createRepoDefinition('app', t.context.app, t.context.global);
				...
				apps: [
					{
						...t.context.app[0],
						path: './',
						errorFile: join(__testDirname, 'cirrus', 'logs', 'app', 'app.error.log'),
